...
# ==========================================
# [세 번째 그래프: 누적관객수 Top 5 다중 선그래프]
# ==========================================
st.header("🏆 누적관객수 Top 5 영화 비교")

# 1) 전체 데이터 기준 누적관객수가 가장 높은 상위 5개 영화 이름 추출
top_5_movies = movie_ranking.head(5)["영화명"].tolist()

# 2) 상위 5개 영화에 해당하는 데이터만 필터링
top_5_df = df[df["영화명"].isin(top_5_movies)]

# 3) Plotly를 이용해 영화별 색상과 범례가 포함된 다중 선그래프 생성
fig3 = px.line(
    top_5_df,
    x="기준일자",
    y="누적관객수",
    color="영화명",  # 영화별로 선 색상을 다르게 지정하고 범례 자동 생성
    title="누적관객수 상위 5개 영화 추이 비교",
...
# 그래프 아래 '이 그래프로 알 수 있는 것' 문구 영역
st.info(
    "💡 **이 그래프로 알 수 있는 것:** 역대 흥행 상위 5개 영화들이 어떤 속도로"
    " 관객을 모았는지 한눈에 비교할 수 있습니다. 가파르게 상승하는 영화와"
    " 완만하게 장기 흥행하는 영화의 패턴 차이를 확인할 수 있습니다."
)
